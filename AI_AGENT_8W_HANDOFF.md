...
> 项目全貌第一遍讲解已完成第 6 周的安全与审计部分，下一步进入测试体系；独立编码能力仍处于第 1 周基础阶段。

## 7. 约定的后续顺序

### 阶段 A：先讲完整体全貌

从当前“安全与审计”之后继续，建议依次讲：

1. 测试体系：单元测试、接口测试、工作流测试分别保护什么。
2. Docker、健康检查、CI 与部署全貌。
3. 作品演示和面试时如何讲清项目。
...
```text
请先读取仓库根目录的 AI_AGENT_8W_HANDOFF.md、LEARNING_README.md、README.md 和 course/README.md，接着当前学习进度继续。我们正在先讲完整个 Agent 项目全貌，已经完成安全与审计的第一轮讲解和 Swagger 实操，下一步从测试体系开始。请用中文、概念优先、少讲不必要的语法；适合用 Swagger 或终端观察的实验优先让我亲自操作。全貌讲完后，再用 1～2 个学习回合快速完成第 1 周 FastAPI CRUD，然后从第 2 周开始正式动手。不要把讲过等同于已经掌握。
```
